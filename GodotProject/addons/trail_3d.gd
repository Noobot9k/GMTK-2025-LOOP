...
enum AlignmentEnum {VIEW, NORMAL, OBJECT}
enum Axis {X, Y, Z}
...
@export var alignment: AlignmentEnum = AlignmentEnum.VIEW
@export var axe: Axis = Axis.Y
...
func _prepare_geometry(point_prev :Point, point :Point, half_width :float, factor :float) -> Array:
	...
	if alignment == AlignmentEnum.VIEW:
	...
	elif alignment == AlignmentEnum.NORMAL:
		if axe == Axis.X:
